...
export default function DashboardStats() {
  ...
  const fetchGlobalStats = async () => {
    try {
      // ✅ CÓDIGO CORREGIDO 1/2
      const response = await fetch(`${window.API_URL}/api/stats-dashboard`, {
  ...
  const aplicarFiltro = async () => {
    ...
    try {
      // ✅ CÓDIGO CORREGIDO 2/2
      const response = await fetch(`${window.API_URL}/api/stats-dashboard?inicio=${filtros.inicio}&fin=${filtros.fin}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

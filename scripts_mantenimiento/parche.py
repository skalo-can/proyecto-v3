...
try:
    ...
    try:
        ...
        print(f"⚠️ estudios.requiere_lectura_radiologo (Puede que ya exista)")

    # 👇 NUEVA COLUMNA DE IA 👇
    try:
        cursor.execute("ALTER TABLE estudios ADD COLUMN prioridad_ia VARCHAR(50) DEFAULT 'NORMAL'")
        print("🤖 Columna 'prioridad_ia' agregada en estudios para Triage.")
    except Exception as e:
        print(f"⚠️ estudios.prioridad_ia (Puede que ya exista)")
    ...
    try:
        ...
        print(f"⚠️ usuarios.intentos_fallidos (Puede que ya exista)")
    ...
    try:
        ...
        print(f"⚠️ usuarios.bloqueado (Puede que ya exista)")
    ...
    try:
        ...
        print(f"⚠️ worklist_orders.study_instance_uid (Puede que ya exista)")

    # Guardamos los cambios
    conn.commit()
    conn.close()
    
    print("\n🚀 PARCHEO FINALIZADO. La base de datos está blindada y lista.")
